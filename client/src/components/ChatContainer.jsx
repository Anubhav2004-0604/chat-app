  ...
  return selectedUser ? (
    <div className=' h-full overflow-scroll relative backdrop-blur-xl'>
      ...
      <div className='flex item-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <p className=' flex-1 text-lg text-white flex item-center gap-2 '>{selectedUser.fullName}
          {onlineUsers.includes(selectedUser._id) && <span className=' w-2 h-2 rounded-full bg-green-500'></span>}
        </p>


        <img onClick={() => setSelectedUser(null)} src={assets.arrow_icon} alt="" className='md:hidden max-w-7' />
        <img src={assets.help_icon} alt="" className='max-md:hidden max-w-7.5' />
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        ...
        <div className='flex-1 flex item-center bg-gray-100/12 px-3 rounded-full'>
          ...
          <label htmlFor="image">
            <img src={assets.gallery_icon} alt=""  className='w-5 mr-2  mt-3 cursor-pointer'/>
          </label>
        </div>
